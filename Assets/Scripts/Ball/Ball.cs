...
public class Ball : MonoBehaviour
{
    ...
    private BallActivation ballActivation;

    // Member data, properties
    public bool isOwnable = true;
    private BallCarrier owner = null;
    ...
    public bool CanBeCarriedBy(BallCarrier ballCarrier) {
        return isOwnable && owner == null;
    }
    public bool CanBeStolenBy(BallCarrier ballCarrier) {
        Debug.Assert(owner != null, "Cannot steal ball if owner is null!");
        bool oppositeTeams = owner.team != ballCarrier.team;
        return isOwnable && oppositeTeams;
    }

    public void ResetBall() {
        owner = null;
        isOwnable = true;
        ballActivation.ResetBall();
        // Set position
    }
    ...
    public void SetOwner(BallCarrier newOwner) {
        owner = newOwner;
        // Case: relinquishing ownership
        if (owner == null) {
            return;
        }
        // Case: setting ownership => check if ball should activate
        if (!owner.IsInNullZone()) {
            ballActivation.RegisterTouch(newOwner);
        }
    }
    public BallCarrier GetOwner() {
        return owner;
    }
...
//             // TODO: rm code that changes mass of ball on possession (that was
//             // leftover logic from when we were considering the ability to whack
//             // other players around with the ball during possession)
//             rigidbody.mass = owner_ == null ? 0.1f : 1000;

//             // TODO: check if this is still needed, or if it's related to the
//             // leftover ball mass logic
